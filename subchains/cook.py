...
def prompt_router(data):
    ...
    data['ingredients'] = '\n'.join(['- '+ingredient for ingredient in data['ingredients']])
    data['steps'] = '\n'.join(['- '+step for step in data['steps']])
    data['recommendations'] = '\n'.join(['- '+recommendation.metadata['recommendation'] \
                               for recommendation in data['recommendations']])
    print(data['recommendations'])
    if data['topic']=="модифікація":
        return ChatPromptTemplate.from_messages([
            ("system", cook_sys_prompt),
            ("user", modification_user_prompt)
        ])
    elif data['topic']=="генерація":
        return ChatPromptTemplate.from_messages([
            ("system", cook_sys_prompt),
            ("user", creation_user_prompt)
        ])
